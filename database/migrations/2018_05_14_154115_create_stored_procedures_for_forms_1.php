...
SET input_values = CONCAT('(','\"',created_at,'\"',',','\"',field_value,'\"');
...
read_loop: LOOP
	...
	ELSE
		IF form_data_id != previous_value THEN
			SET input_values = CONCAT(input_values, ')',',(','\"',created_at,'\"',',');
...
read_loop: LOOP
	...
	ELSE
		IF @list_of_fields = '' THEN
			SET @list_of_fields = CONCAT('(','`created_at`,','`',a,'`'); 
        ELSE 
...
CALL insert_data_into_temporary_form_table('temp',form_key_value, @list_of_fields);
-- SELECT * FROM temp;
CLOSE key_cursor;
